...
(function($){
    $.fn.uploadify = function(opts){
        ...
        var defaults = {
            file_types:'',//允许上传的文件类型，格式'*.jpg;*.doc'
            uploader:'',//文件提交的地址
            auto:true,//是否开启自动上传
            method:'post',//发送请求的方式，get或post
            multi:true,//是否允许选择多个文件
            formData:null,//发送给服务端的参数，格式：{key1:value1,key2:value2}
            fileObjName:'file_upload',//在后端接受文件的参数名称，如PHP中的$_FILES['file']
        ...
        this.each(function(){
            ...
            inputStr += ' accept="';
            inputStr += getFileTypes(option.file_types).join(",");
            inputStr += '"/>';
            ...
            var fileObj = {
                ...
                filter: function(files) {		//选择文件组的过滤方法
                    var arr = [];
                    var typeArray = getFileTypes(option.file_types);
